...
char *strReplace(char *src, const char *oldsubstr, const char *newsubstr)
{
     char *needle;
    char *tmp;

     if (strlen(oldsubstr) == strlen(newsubstr) && strcmp(oldsubstr, newsubstr) == 0)
    { 
        return 0;
    }
while ((needle = strstr(src, oldsubstr)))
    {
        tmp = (char*)malloc(strlen(src) + (strlen(newsubstr) - strlen(oldsubstr)) +1);
        strncpy(tmp, src, needle-src);
        tmp[needle-src] = '\0';
        strcat(tmp, newsubstr);
        strcat(tmp, needle+strlen(oldsubstr)); 
        src = strdup(tmp); 
        free(tmp); 
    }
    return src;
