"""
This module implements probabilistic data structure
which is able to calculate the cardinality of large
multisets in a single pass using little auxiliary memory
"""
...
def estimate_bias(E, p):
    ...
    return sum([float(bias_vector[i])
               for i in nearest_neighbors]) / len(nearest_neighbors)


def get_nearest_neighbors(E, estimate_vector):
    distance_map = [((E - float(val)) ** 2, idx)
...
class HyperLogLog(object):
    ...
    def add(self, value):
        ...
        term1 = self.M[j]
        term2 = get_rho(w, 64 - self.p)
        self.M[j] = max(term1, term2)
    ...
    def update(self, *others):
        ...
        self.M = [max(*items) for items in
                  zip(*([item.M for item in others] +
                      [self.M]))]
    ...
    def _Ep(self):
        E = self.alpha * float(self.m ** 2) / sum(math.pow(2.0, -x)
                                                  for x in self.M)
