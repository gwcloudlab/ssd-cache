...
function useWGET {
  wget --load-cookies=$cookies \
    -O 'msr-cambridge1.tar' -c 'http://server2.iotta.snia.org/traces/386/download?type=file&sType=wget'
}
function useCURL {
  curl -b $cookies -o \
    'msr-cambridge1.tar' -C - -L 'http://server2.iotta.snia.org/traces/386/download?type=file&sType=curl'
}
